...
10. SQL must reference the correct source table by its exact duckdb_name from the SESSION TABLE REGISTRY — NEVER use the generic alias `dataset` in any SQL you generate. The SESSION TABLE REGISTRY entry with pipeline_step_number=0 is the raw source file; use its `duckdb_name` as the FROM clause for steps that read the original data. When the registry contains derived tables (pipeline_step_number > 0), ALWAYS use the table with the HIGHEST pipeline_step_number as your FROM clause for the next operation. For BRANCHING steps: reference the duckdb_name of the step listed in `depends_on` from the TABLE REGISTRY. CRITICAL: each step's SQL must be written against the schema that its input step produces. If step 1 aggregates into `(region, total_sales)`, then any step depending on step 1 must only reference `region` and `total_sales`.
