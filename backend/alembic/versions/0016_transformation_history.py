...
def upgrade() -> None:
    ...
    if not inspector.has_table("transformation_history"):
        op.create_table(
    ...
    existing_indexes = {
        idx.get("name")
        for idx in inspector.get_indexes("transformation_history")
        if idx.get("name")
    }

    if "idx_transformation_history_dataset" not in existing_indexes:
        op.create_index(
            "idx_transformation_history_dataset",
            "transformation_history",
            ["dataset_id"],
        )
    if "idx_transformation_history_user" not in existing_indexes:
        op.create_index(
